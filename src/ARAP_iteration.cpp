    ...
    const Eigen::MatrixXd &L_initial)
{
    ...
    for (const auto &c : constraint_indices)
    {
        ...
        // for each constraint, retrieve the constraint vector
        Eigen::Vector3d constraint = V.row(c);

        for (unsigned int i = 0; i < vertices; ++i)
        {
            // for each vertex i check if the system matrix coefficient at position (i, c) is not zero
            if (L.coeff(i, c) != 0.0)
            {
                ...
                // the (i, c) position corresponds to the interaction between the vertex i and the constrained degree of freedom represented by c
                b_x(i) -= L.coeff(i, c) * constraint.x();
                b_y(i) -= L.coeff(i, c) * constraint.y();
                b_z(i) -= L.coeff(i, c) * constraint.z();
            }
        }
        ...
        for (unsigned int i = 0; i < vertices; ++i)
        {
            if (L.coeff(c, i) != 0.0)
            {
                // set the coefficient at position (c, i) and (i, c) to zero
                L(c, i) = 0.0;
                L(i, c) = 0.0;
                L(c, c) = 1.0;
            }
